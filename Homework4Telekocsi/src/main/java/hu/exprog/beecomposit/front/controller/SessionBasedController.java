package hu.exprog.beecomposit.front.controller;
...
public class SessionBasedController {
    ...
    public void setInitializedController(String conrollerName) {
        Object session = FacesContext.getCurrentInstance().getExternalContext().getSession(false);
        if (session instanceof HttpSession) {
            ((HttpSession) session).setAttribute("initializedController", conrollerName);
        }
    }
}
