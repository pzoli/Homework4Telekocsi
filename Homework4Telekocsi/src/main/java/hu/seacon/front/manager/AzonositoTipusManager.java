...
@Named
@WindowScoped
public class AzonositoTipusManager extends BasicManager<AzonositoTipus> implements Serializable {
	...
	public void initValue() {
		if ((current == null) || (!current.isPresent()) || current.get().getId() != null) {
			current = Optional.of(new AzonositoTipus());
			clearProperties();
		}
	}
		...
		try {
			if (current.isPresent()) {
				if (current.get().getId() == null) {
					azonositoTipusService.persist(current.get());
				} else {
					azonositoTipusService.merge(current.get());
				}
			}
			current = Optional.of(new AzonositoTipus());
			clearProperties();
	...
	@Override
	protected BasicService<AzonositoTipus> getService() {
		return azonositoTipusService;
	}
	...
	@Override
	public boolean checkEditableRights(AzonositoTipus entity) throws ActionAccessDeniedException {
		return true;
	}

	@Override
	public boolean checkDeleteRight(AzonositoTipus entity) throws ActionAccessDeniedException {
		return true;
	}
