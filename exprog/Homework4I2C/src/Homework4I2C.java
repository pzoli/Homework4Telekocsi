 ...
 *
 * @author Alexander Barker (JNativeHook), Zoltan Papp (Arduino serial communication)
 ...
 */
public class Homework4I2C extends JFrame implements ActionListener, ItemListener, NativeKeyListener,
		NativeMouseInputListener, NativeMouseWheelListener, WindowListener, jssc.SerialPortEventListener {
	...
	private JCheckBoxMenuItem menuItemEnable, menuItemKeyboardEvents, menuItemButtonEvents, menuItemMotionEvents,
			menuItemWheelEvents;
	...
	private Integer mouseOldX;
	private Integer mouseOldY;
	private Robot robot;
	 ...
	 */
	public Homework4I2C(String serialPort) {
		...
		menuItemEnable
				.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_H, ActionEvent.CTRL_MASK + ActionEvent.SHIFT_MASK));
		...
		menuItemMotionEvents
				.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_M, ActionEvent.CTRL_MASK + ActionEvent.SHIFT_MASK));
	 ...
	 *
	 * @param e
	 *            the native input event to display.
	 */
	...
	/**
	 * Display information about the native keyboard and mouse along with any errors
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				if (portNames.length == 1) {
					new Homework4I2C(portNames[0]);
				} else {
					new Homework4I2C(null);
				}
			}
		});
	}
	...
	private final class LogFormatter extends Formatter {
		...
		public String format(LogRecord record) {
			...
			line.append(new Date(record.getMillis())).append(" ").append(record.getLevel().getLocalizedName())
	...
	/**
	 * Open serial port
	 * @param serialPortName Serial port (example: windows - COM7)
	 */
	public void initSerial(String serialPortName) {
		if (serialPort == null) {
	...
	/**
	 * Send mouse move delta to Arduino Leonardo
	 * @param e native mouse event
	 */
	private void sendSerialMessage(NativeMouseEvent e) {
		if (serialPort != null && serialPort.isOpened()) {
			try {
				int delatX, delatY;
				if (mouseOldX != null) {
					delatX = mouseOldX - e.getX();
				} else {
					delatX = 0;
				}
				mouseOldX = e.getX();
				if (mouseOldY != null) {
					delatY = mouseOldY - e.getY();
				} else {
					delatY = 0;
				}
				mouseOldY = e.getY();
				serialPort.writeString("mouse=m;" + -1 * delatX + ";" + -1 * delatY + '\n');
		...
		if (event.isRXCHAR()) {
			...
			if (count > 0) {
				try {
					...
					Logger.getLogger(this.getClass().getName()).log(Level.INFO, input);
				} catch (SerialPortException ex) {
	...
	/**
	 * Finish program, close opened port
	 */
	@Override
